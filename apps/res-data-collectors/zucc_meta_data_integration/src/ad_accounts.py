...
def api_call_function(id, api, fields, params):
    ...
    return AdAccount(fbid=id, api=api).api_get(
        fields=list(fields),
        params=params,
    )
...
if (
    ...
    # This endpoint has no parameters other than limit; always a full sync
    logger.info(
        f"Meta sync for ad accounts{single_account_str if single_account_str else ''}"
    )
    ...
    logger.info("Meta ad accounts sync complete")
