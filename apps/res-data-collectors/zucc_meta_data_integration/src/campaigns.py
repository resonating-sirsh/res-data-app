...
def api_call_function(id, api, fields, params):
    ...
    return AdAccount(fbid=id, api=api).get_campaigns(
        fields=fields,
        params=params,
    )
...
if (
    ...
    fields = {
        ...
        "spend_cap",
        "status",
        # Updating spend_cap, daily_budget, or lifetime_budget will not
        # update this field
        "updated_time",
    }
    ...
    if SYNC_TYPE == "incremental":
        ...
        if since_ts is None:
            ...
            logger.warning(
                "Error during latest date retrieval; defaulting to Meta full sync for ad campaigns"
            )

            # Only retrieve ad campaigns updated since July 2017. It's unlikely ad campaigns not
        ...
        if SYNC_TYPE == "full":
            ...
            logger.warning(
                "No or invalid sync type provided; defaulting to Meta full sync for ad campaigns"
            )
    ...
    logger.info(
        f"Meta {SYNC_TYPE} sync for ad campaigns{single_account_str if single_account_str else ''}"
    )
    ...
    logger.info("Meta ad campaigns sync complete")
