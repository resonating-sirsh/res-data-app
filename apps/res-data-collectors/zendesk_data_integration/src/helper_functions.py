...
def get_latest_snowflake_epoch(
    database,
    schema,
    table,
    snowflake_user,
...
async def get_api_data(
    ...
    username = api_cred_dict["username"]
    api_key = api_cred_dict["api_key"]
    subdomain = api_cred_dict["subdomain"]
    unencoded_str = f"{username}/token:{api_key}"
    base64_encoded_str = (base64.b64encode((unencoded_str).encode("ascii"))).decode(
        "ascii"
    )
    headers = {"Authorization": f"Basic {base64_encoded_str}"}
    data = []

    # Iterate over pages until a response returns end_of_stream = True. For he
    ...
    url = f"https://{subdomain}.zendesk.com{endpoint}"

    while not is_end_of_stream:

        # Get first page of data for the endpoint
        async with aiohttp_session.get(
            ...
            try:
                ...
                page_data = await response.json(encoding="UTF-8")
                data.extend(page_data[data_key])

                # Iterate url; null params. Using [] to raise exceptions if the
                # utilized keys are missing in response (they never should be)
                is_end_of_stream = page_data["end_of_stream"]
                url = page_data[pagination_key]
                params = None
    ...
    for record in data:
        ...
        record["synced_at"] = sync_time

    target_output.extend(data)
...
async def get_all_api_data(
    ...
    async with aiohttp.ClientSession(
        ...
        futures = [
            ...
            for api_cred_dict in api_cred_dicts
        ]
