...
from sqlalchemy.types import BOOLEAN, DATE, INTEGER, NUMERIC, TIMESTAMP, VARCHAR
...
def initialize_analytics_reporting(
    google_credentials: dict, scopes, api, version_string
):

    # Initializes an Analytics Reporting API V4 service object. Returns an
    # authorized Analytics Reporting API V4 service object.
    ...
    analytics = build(f"{api}", f"{version_string}", credentials=credentials)
...
def camel_to_snake(input_text):
    ...
    snake_string = re.sub(r"(?<!^)(?=[A-Z])", "_", input_text).lower()
