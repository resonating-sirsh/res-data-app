...
def get_mongo_one_measurements(
    client: MongoClient, ts_str: str, snowflake_creds, database
):
    """
    Retrieves ONE measurements from MongoDB -- the resmagic database and
    oneMeasurements collection. These measurements are recorded within the sew
    node and include errors as well as points of measure.
    ...
    try:
        ...
        log_str = (
            f"Retrieving ONE measurements from MongoDB since {ts_str}"
            if ts_str
            else "Retrieving all ONE measurements from MongoDB"
        )
        logger.info(log_str)
        sync_time = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S.%f %Z")

        if ts:
            cursor = client.resmagic.oneMeasurements.find(
                {
                    "$or": [
                        {"updatedAt": {"$gt": ts_str}},
    ...
    try:
        to_snowflake(
            data, snowflake_creds, database, "mongodb", "resmagic_one_measurements"
        )
        data.clear()
...
if (
    ...
    get_mongo_one_measurements(client, ts, snowflake_creds, database)
