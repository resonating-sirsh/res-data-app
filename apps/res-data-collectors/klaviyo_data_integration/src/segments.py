...
if (
    ...
    output_data = asyncio.run(get_all_api_data(klaviyo_api_keys, "segments", params))

    # Construct a dictionary where each key is an account ID and each value is a
    # list of that account's segment IDs
    segment_ids = {}

    for rec in output_data["segments"]:
        account_id = rec["account_id"]
        segment_id = rec["id"]

        # Add the account ID as a key if it is not there yet
        if account_id not in segment_ids:
            segment_ids[account_id] = []

        # Add the record's segment ID to its account's list
        segment_ids[account_id].append(segment_id)

    # Set new params for segment profiles query. Profiles data is retrieved for
    # all profiles in another step so this table of segment membership doesn't
    # need full details for each profile beyond IDs and join timestamps
    profile_fields = ["joined_group_at"]
    params = {
        "fields[profile]": ",".join(profile_fields),
        "page[size]": "100",
    }
    logger.info("Retrieving segment profiles")
