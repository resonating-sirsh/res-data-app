...
if (
    ...
    # Set params according sync type
    params = {
        "filter": f"greater-or-equal(updated,{ts})" if ts else "",
    }

    # Execute program
    output_data = asyncio.run(get_all_api_data(klaviyo_api_keys, "templates", params))[
        "templates"
    ]

    if len(output_data) > 0:
        logger.info(f"Syncing")
        to_snowflake(output_data, snowflake_creds, database, "klaviyo", "templates")
