...
FROM 
SELL.BRANDS b
INNER JOIN sell.subscriptions s on s.brand_id = b.id
WHERE s.deleted_from IS NULL or s.deleted_from > CURRENT_TIMESTAMP;
"""
# nudge nudge
...
# { ..
#           or: [
#             {wasPaymentSuccessful: {is: false}},
#             {wasPaymentSuccessful: {is: null}}
#           ]
#         },
  ...
  orders(
   ...
   where: {
      and: [
        ...
        {createdAt: {isGreaterThan: "2024-03-23"}}
      ]
    }
  ) {
    count
...
class DelayedPaymentResolver:
    ...
    def process(self):
        result_intent = None
        logger.info("Start delayed payment run process. ver 22 mar..")
        ...
        for brand_info in brands_list:
            ...
            try:
                ...
                while orders_first_iteration or count_orders < total_orders:
                    ...
                    order_update_data["orders"]["orders"] = [
                        order
                        for order in order_update_data["orders"]["orders"]
                        if order["code"] not in ["2148542", "2148601"]
                    ]  # pure hack, there is no cancellation info i can use! i hate airtable and graphQL. JFC

                    order_update_data["orders"]["orders"] = [
                        order
                        for order in order_update_data["orders"]["orders"]
                        if order["salesChannel"].lower() != "first one"
                    ]
                    ...
                    if order_update_data and order_update_data["orders"]:
                        total_orders = len(order_update_data["orders"]["orders"])
