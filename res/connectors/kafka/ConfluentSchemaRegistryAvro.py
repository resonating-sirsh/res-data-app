...
# RESONANCE NOTE: this is forked from https://docs.confluent.io/5.5.0/clients/confluent-kafka-python/_modules/confluent_kafka/schema_registry/avro.html
# The function _schema_loads wasn't working with schemas starting with "[", so we have to override this function
...
class AvroSerializer(Serializer):
    ...
    def __init__(self, schema_str, schema_registry_client, to_dict=None, conf=None):
        ...
        if len(conf_copy) > 0:
            raise ValueError(
                "Unrecognized properties: {}".format(", ".format(conf_copy.keys()))
            )
    ...
    def __call__(self, value, ctx):
        ...
        if len(value) <= 5:
            raise SerializationError(
                "Message too small. This message was not"
        ...
        with _ContextStringIO(value) as payload:
            ...
            if magic != _MAGIC_BYTE:
                raise SerializationError(
                    "Unknown magic byte. This message was"
            ...
            obj_dict = schemaless_reader(payload, writer_schema, self._reader_schema)
